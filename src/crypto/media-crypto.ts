...
/**
 * Derive the per-broadcast AES-GCM content key and release queued frames.
 *
 *   CK = HKDF-SHA256(
 *          IKM  = fragmentKey,               // 32 random bytes, lives ONLY in the #k= share-link fragment
 *          salt = globalSalt ‖ streamSalt,   // operator kill-switch ‖ broadcaster-rotatable salt
 *          info = "earthseed-media-v1|" + streamId + "|" + epoch
 *        )
 *
 * The fragment key never reaches any server (it is in the URL #fragment, which
 * browsers do not transmit); the salts are served by the app but decode nothing
 * on their own. So no middleman — relay, broker, fleet, or app host — can derive
 * CK. Rotating either salt (or bumping epoch) yields a fresh CK: the operator's
 * global kill switch re-keys every stream at once; a broadcaster's reset re-keys
 * just theirs. Viewers re-derive when {@link deriveMediaKey} is called again with
 * the new salt/epoch (the epoch change is the signal to re-derive).
 */
export interface DeriveParams {
  fragmentKeyB64: string; // IKM: 32-byte key from the #k= link fragment
  globalSaltB64: string; // operator-controlled site-wide kill-switch salt
  streamSaltB64: string; // per-broadcast, broadcaster-rotatable salt
  streamId: string; // bound into HKDF info so keys can't cross streams
  epoch: number; // rotation counter; bound into info so each reset is a distinct key
}

let derivedEpoch: number | null = null;

/** The epoch of the key currently installed (or null if none) — lets the viewer skip redundant re-derives. */
export function currentEpoch(): number | null {
  return derivedEpoch;
}

export async function deriveMediaKey(p: DeriveParams): Promise<void> {
  const ikmBytes = b64urlToBytes(p.fragmentKeyB64);
  const globalBytes = b64urlToBytes(p.globalSaltB64);
  const streamBytes = b64urlToBytes(p.streamSaltB64);
  const salt = new Uint8Array(globalBytes.byteLength + streamBytes.byteLength);
  salt.set(globalBytes, 0);
  salt.set(streamBytes, globalBytes.byteLength);
