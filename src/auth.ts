...
export async function getCurrentUser(): Promise<{ user: User | null; geo: Geo | null }> {
  // Tier-2 two-file mode has no accounts and no earthseed Worker /api — and on a
  // file:// page a relative /api fetch is a CORS error. Skip it entirely there.
  if (typeof location !== "undefined" && (location.protocol === "file:" || document.querySelector('meta[name="earthseed-mode"]'))) {
...
// --- Media-key salts ---------------------------------------------------------
// The AES content key is derived in the browser as HKDF(#k= fragment,
// globalSalt ‖ streamSalt). These helpers fetch the salts (both sides) and let a
// broadcaster set/rotate the per-stream salt. Salts decode nothing on their own,
// so GET is public; PUT is gated by a rotate secret the broadcaster keeps locally.
export interface SaltInfo {
  global: string; // operator kill-switch salt (base64url)
  stream: string | null; // per-broadcast salt (null until the broadcaster sets it)
  epoch: number; // bumped on each rotation; the signal for viewers to re-derive
}

export async function getSalt(streamId: string): Promise<SaltInfo | null> {
  try {
    const response = await fetch(`/api/salt/${streamId}`);
    if (!response.ok) return null;
    const data = await response.json();
    return { global: data.global, stream: data.stream ?? null, epoch: data.epoch ?? 0 };
  } catch {
    return null;
  }
}

// Set/rotate this stream's salt. `secret` is the broadcaster's rotate secret (held
// only in their browser); the Worker binds it on first write and requires it after.
export async function putSalt(streamId: string, stream: string, secret: string): Promise<SaltInfo | null> {
  try {
    const response = await fetch(`/api/salt/${streamId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    if (!response.ok) {
      console.error("putSalt failed:", response.status, await response.text());
      return null;
    }
    const data = await response.json();
    return { global: data.global, stream: data.stream ?? null, epoch: data.epoch ?? 0 };
