// Patched subscribe.js for Cloudflare compatibility
// Accepts filter types 1 (LatestGroup) and 2 (LatestObject)

import * as Message from "../ietf/message.js";
import * as Namespace from "../ietf/namespace.js";
import { Parameters } from "../ietf/parameters.js";

// We send Latest Group (0x1)
...
export class Subscribe {
    ...
    trackName;
    subscriberPriority;
    filterType; // Store the filter type

    constructor(requestId, trackNamespace, trackName, subscriberPriority, filterType = FILTER_TYPE) {
    ...
    static async #decode(r) {
        ...
        // PATCHED: Accept filter types 1 (LatestGroup) and 2 (LatestObject)
        if (filterType !== 0x01 && filterType !== 0x02) {
            throw new Error(`unsupported filter type: ${filterType}`);
        }
        await Parameters.decode(r); // ignore parameters
        return new Subscribe(requestId, trackNamespace, trackName, subscriberPriority, filterType);
    }
}
