...
const serverStatus: ServerStatus = {
  ...
  // No relay is chosen until go-live/watch assigns one (setActiveRelay) — show a neutral
  // placeholder instead of a hardcoded host, which would otherwise misreport the backend
  // (e.g. the relay is only known once the Worker returns it; in static mode it's Cloudflare).
...
// ?jwt= only when present — open relays (e.g. the Cloudflare draft-14 interop relay, used in
// static FLEET_MODE) need no token, so we omit it. A redundant ":443" is stripped so the URL
// is the bare https origin. Media stays end-to-end encrypted regardless of the token.
...
// layer and nothing more: "media encrypted" is an INFRASTRUCTURE property (the media
...
// "media encrypted" — shown on EVERY stream (encryption is mandatory). States that the
...
import { armPublisher, armViewer, deriveMediaKey, resetMediaKey } from "./crypto/media-crypto";
import {
  getOrCreateFragmentKey,
  getOrCreateRotateSecret,
  newStreamSalt,
  fragmentKeyFromHash,
} from "./crypto/link-keys";
import { getOrCreateNode } from "./crypto/node-id";
import { publishDhtRecord, resolveDhtRecord } from "./crypto/dht";
// Tier-2: provisioning + salts go straight to the broker (tinymoq.com) with the
// public pk_ — no earthseed Worker /api/* in the path. getSalt/putSalt keep the
// same shape as the old auth.ts helpers, so the call sites are unchanged.
import { assignPublish, assignWatch, getSalt, putSalt } from "./broker";
...
async function getRouteInfo(): Promise<{ view: View; streamId: string }> {
  ...
  // Tier-2 two-file mode: send.html / view.html declare their role via a meta tag,
  // so routing does NOT depend on the URL path — which is what makes them work from
  // a double-clicked file:// (where the path is meaningless) and from any host.
  const mode = document.querySelector('meta[name="earthseed-mode"]')?.getAttribute("content");
  if (mode === "broadcast") return { view: "broadcast", streamId: "" };
  if (mode === "watch") {
    const n = new URLSearchParams(window.location.search).get("node");
    return { view: "watch", streamId: n && /^[a-z2-7]{52}$/.test(n) ? n : "" };
  }
  ...
  // Watch-by-pubkey: /?node=<52-char base32 ed25519 pubkey>. This is the ONLY watch path —
  ...
  if (node && /^[a-z2-7]{52}$/.test(node)) {
    return { view: "watch", streamId: node };
  }

  // Broadcast view: "/". The view mints (or loads) the broadcaster's persistent node
  // identity itself; there is no ?stream= id to generate.
...
async function initBroadcastView() {
  // Watch-by-pubkey: mint (or load) the broadcaster's PERSISTENT node identity. The ed25519
  // pubkey IS the stream's identity + DHT discovery handle, and (bare) the moq broadcast track
  // name. No login required — discovery is the DHT and the fleet is token-gated per broadcast.
  const node = await getOrCreateNode();
  const streamId = node.id; // the 52-char pubkey; reused as the local key for salt/fragment
  const streamName = streamId; // bare nodeId is the moq track name on the node path
  // The content key is minted here in the browser and carried ONLY in the #k= fragment of the
  // share link — it never reaches any server (browsers don't send the fragment). Both sides
  // derive CK = HKDF(fragmentKey, globalSalt ‖ streamSalt). Persisted in localStorage (keyed by
  // the node id) so the link + stream ownership survive a reload.
  const fragmentKey = getOrCreateFragmentKey(streamId);
  const rotateSecret = getOrCreateRotateSecret(streamId);
  // Share link points at view.html (relative to this page, so it works on any host AND from a
  // local file://): ?node=<pubkey> for DHT discovery, #k=<key> for decryption. If this page is
  // running under a URL-supplied ?key= (a non-default public tier), carry it forward so the
  // viewer's broker call lands on the SAME fleet as the broadcaster.
  const urlKey = new URLSearchParams(window.location.search).get("key");
  const keyQuery = urlKey && urlKey.startsWith("pk_") ? `&key=${encodeURIComponent(urlKey)}` : "";
  const shareUrl = `${new URL("view.html", window.location.href).href}?node=${streamId}${keyQuery}#k=${fragmentKey}`;

  console.log(`Earthseed Broadcast (node) - ${streamId}`);
  ...
  // Node/DHT path has no D1 stream settings — hide the require-auth + chat toggles.
  document.querySelectorAll("#broadcast-view .require-auth-toggle").forEach((el) => (el as HTMLElement).classList.add("hidden"));

  // Update the page with the node id (long — let it wrap).
  const streamDisplay = document.getElementById("stream-id");
  const copyBtn = document.getElementById("copy-btn");

  if (streamDisplay) {
    // The node id (public key) is 52 chars — show a short prefix with the full value on hover.
    // It's the shareable discovery handle, not a secret; the copy button copies the full link.
    streamDisplay.textContent = `${streamId.slice(0, 5)}…`;
    streamDisplay.title = streamId;
    (streamDisplay as HTMLElement).style.wordBreak = "normal";
    (streamDisplay as HTMLElement).style.cursor = "help";
  }
  ...
  // Stream-header indicator: "Relay-blind" states the INFRASTRUCTURE property (relay/server/
  // broker/fleet only ever move ciphertext — none can derive the content key). The node/DHT
  // path has no D1 access policy, chat, or viewer stats, so those controls are omitted.
  const streamHeaderEl = document.querySelector(".stream-header");
  if (streamHeaderEl) {
    streamHeaderEl.appendChild(createRelayBlindBadge());
    streamHeaderEl.appendChild(createSecurityDetails());
  }
  ...
  if (publisher) {
    ...
    // Tracks the global kill-switch salt this broadcast derived with, so an operator
    // rotation of salt:global (in KV) re-keys the LIVE broadcast (not just new ones).
    let lastGlobalSalt: string | null = null;
    let globalPollStarted = false;

    // First device selection = go live (watch-by-pubkey):
    //   1) derive the content key from #k= + this stream's salt (encrypt from the first byte),
    //   2) /api/publish → assign an origin + read its iroh EndpointId (Worker holds the bearer),
    //   3) connect the publisher to that origin (gated by a put-scoped jwt),
    //   4) self-sign the DHT record (our pubkey → origin EndpointId) so viewers can discover us.
    // Broadcaster "stand by" status — go-live has a few network steps (the DHT publish
    // can take a few seconds), so surface each phase instead of a silent wait.
    const goLiveStatusEl = document.getElementById("go-live-status");
    const setGoLiveStatus = (text: string | null, state: "busy" | "live" | "error" = "busy") => {
      if (!goLiveStatusEl) return;
      goLiveStatusEl.className = text ? `go-live-status ${state}` : "go-live-status hidden";
      goLiveStatusEl.textContent = text ?? "";
    };
    ...
    const goLive = (): Promise<void> => {
      ...
      goLivePromise = (async () => {
        try {
          setGoLiveStatus("Setting up encryption…");
          // 1) Encryption. Mint + claim this stream's salt (gated by our rotate secret), then
          //    derive CK = HKDF(fragmentKey, globalSalt ‖ streamSalt). The key never leaves this
          //    browser except as the #k= link fragment; the server sees only the salts.
          const salt = newStreamSalt();
          const saltInfo = await putSalt(streamId, salt, rotateSecret);
          if (!saltInfo || !saltInfo.stream) {
            console.error("[crypto] could not set the stream salt; not going live");
            setGoLiveStatus("Couldn't go live — tap a capture button to retry", "error");
            publisher.source = null;
            setActiveRelay(null);
            goLivePromise = null;
            return;
          }
          armPublisher(); // idempotent
          await deriveMediaKey({
            fragmentKeyB64: fragmentKey,
            globalSaltB64: saltInfo.global,
            streamSaltB64: saltInfo.stream,
            streamId,
            epoch: saltInfo.epoch,
          });
          lastGlobalSalt = saltInfo.global;
          console.log(`[crypto] media key derived (epoch ${saltInfo.epoch})`);

          // 2) Provision an origin + read its iroh EndpointId — straight from the broker
          //    (tinymoq.com/cdn/assign) with the public pk_; the broker mints the publish token.
          setGoLiveStatus("Reserving a relay…");
          const pub = await assignPublish(streamId);
          if ("error" in pub) {
            console.error("[node-publish] broker assign failed:", pub.error);
            setGoLiveStatus("Couldn't reach a relay — tap a capture button to retry", "error");
            setActiveRelay(null);
            goLivePromise = null;
            return;
          }
          // 3) Connect (relay_url ends in "/", so "?jwt=" appends cleanly).
          publisher.setAttribute("url", pub.jwt ? `${pub.relay_url}?jwt=${pub.jwt}` : pub.relay_url);
          // 4) Publish the DHT discovery record: our key → this origin's EndpointId. Without it
          //    a viewer has nothing to resolve, so a failure fails go-live. (Can take a few seconds.)
          setGoLiveStatus("Publishing to the discovery network…");
          await publishDhtRecord(node.keyPair, streamId, pub.origin_endpoint_id);
          setActiveRelay(pub.relay_url);
          isLive = true;
          setGoLiveStatus("Live — copy your share link to invite viewers", "live");
          console.log("[node-publish] live:", pub.relay_url, "eid:", pub.origin_endpoint_id, "name:", streamId);

          // 5) Operator global-salt kill-switch watch — re-key the live broadcast in sync so
          //    viewers who re-fetch the new global keep playing.
          if (!globalPollStarted) {
            globalPollStarted = true;
            const globalPoll = window.setInterval(async () => {
              if (!isLive) return;
              const s = await getSalt(streamId);
              if (s && s.stream && s.global !== lastGlobalSalt) {
                lastGlobalSalt = s.global;
                await deriveMediaKey({
                  fragmentKeyB64: fragmentKey,
                  globalSaltB64: s.global,
                  streamSaltB64: s.stream,
                  streamId,
                  epoch: s.epoch,
                });
                console.log("[crypto] operator rotated the global salt; broadcaster re-derived");
              }
            }, 5000);
            window.addEventListener("beforeunload", () => clearInterval(globalPoll));
          }
        } catch (e) {
          console.error("[node-publish] go-live failed:", e);
          setGoLiveStatus("Couldn't go live — tap a capture button to retry", "error");
          setActiveRelay(null);
          goLivePromise = null; // allow retry on the next capture toggle
        }
      })();
      return goLivePromise;
    };

    // End the broadcast: allow re-assign on the next device action. No D1 event to close; the
    // fleet reaps the origin when the publish connection drops.
    const endBroadcast = () => {
      isLive = false;
      goLivePromise = null;
      setGoLiveStatus(null);
      // Drop the content key (keep the publisher armed): a restarted broadcast derives a fresh
      // key, and frames queue until it arrives — never encrypted with the previous session's key.
      resetMediaKey();
    };

    // "Reset key" — the broadcaster's kill switch for THIS stream. Mint a fresh salt,
    // rotate it server-side (bumping the epoch), and re-derive CK for future frames.
    // Anyone still holding the old link keeps their #k= fragment but can no longer
    // decode new frames unless they re-fetch the new salt — which, once salt delivery
    // is gated (future auth), makes this a true revocation. (Operator-wide revocation
    // is the global kill-switch salt, rotated in KV: salt:global.)
    const resetKeyBtn = document.getElementById("reset-key-btn");
    resetKeyBtn?.addEventListener("click", async () => {
      if (!isLive) return; // only meaningful once live
      const salt = newStreamSalt();
      const saltInfo = await putSalt(streamId, salt, rotateSecret);
      if (!saltInfo || !saltInfo.stream) {
        console.error("[crypto] key reset failed");
        return;
      }
      await deriveMediaKey({
        fragmentKeyB64: fragmentKey,
      ...
      lastGlobalSalt = saltInfo.global;
      console.log(`[crypto] key reset (epoch ${saltInfo.epoch}) — old viewers must re-fetch the salt to keep watching`);
      resetKeyBtn.classList.add("reset-done");
      setTimeout(() => resetKeyBtn.classList.remove("reset-done"), 1500);
    });
    ...
    makeToggle("camera", ICONS.camera, "Camera");
    makeToggle("audio", ICONS.audio, "Audio (microphone; also mixes in tab/system audio when screen sharing)");
    makeToggle("screen", ICONS.screen, "Screen");
    ...
    stopBtn.title = "Stop";
    stopBtn.innerHTML =
      '<svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18" aria-hidden="true"><rect x="5" y="5" width="14" height="14" rx="2"/></svg>';
    ...
    // (Node/DHT path: no D1 broadcast event to close on unload; the fleet reaps the origin
    // when the publish connection drops.)

    // (HTML overlay editor removed — it wrote to D1 via the old stream-id path and is not
    // wired on the node/DHT flow; viewers never load it.)
  }
...
// doesn't). The edge resolves the tenant by the token's kid, validates it against earthseed's
// verify_jwk, and requires a valid subscribe <broadcast> scope. No provisioning bearer ever
// enters the browser → relay-blind preserved. `origin`/`pull` are added ONLY for cross-pull
// (edge pulls the broadcast from the publisher's origin relay); in standalone mode the worker
// omits edgeHost/pullToken because the publisher is already on the edge. The response body is
// the EDGE's media endpoint "host:port" (a fresh erik.moqcdn.net:<port> per stream) as plain
// text — some builds wrap it as JSON {relay}, so we accept both. The browser MUST dial that
// returned value (NOT `origin`, which is only the upstream the edge pulls from). The same
// watch token then drives the QUIC connect. Returns null on any failure → caller falls to B/A.
...
// Resolve a node id -> { relayUrl, broadcast, jwt } via the two-step watch-by-pubkey path:
//   1. DISCOVERY (browser): read the broadcaster's signed record off the Mainline DHT to learn
//      the origin relay's iroh EndpointId. No directory server.
//   2. PROVISIONING (Worker): hand that EndpointId to /api/edge, which places an edge on the
//      viewer's nearest fleet that iroh-pulls the origin. The fleet bearer stays in the Worker.
// Returns null when nothing is published yet (poll) or provisioning failed.
async function resolveNodeId(
  nodeId: string
): Promise<{ relayUrl: string; broadcast: string; jwt?: string } | null> {
  try {
    const rec = await resolveDhtRecord(nodeId);
    if (!rec) return null; // not published yet — caller polls
    // Provision an edge straight from the broker with the public pk_; the broker geo-routes
    // from the browser's own request.cf (no forwarding) and mints the subscribe token.
    const edge = await assignWatch(nodeId, rec.originEndpointId);
    if ("error" in edge) return null;
    return { relayUrl: edge.relay_url, broadcast: rec.name, jwt: edge.jwt ?? undefined };
  } catch {
    return null;
  }
}

async function initWatchView(nodeId: string) {
  console.log(`Earthseed Watch (node) - ${nodeId}`);
  ...
  document.getElementById("new-stream-btn")?.classList.add("hidden");

  // view.html opened without a valid ?node= (e.g. a bare double-click) has nothing to
  // resolve — point the visitor at a full share link instead of spinning forever.
  if (!/^[a-z2-7]{52}$/.test(nodeId)) {
    const section = document.querySelector("#watch-view section");
    if (section) {
      const msg = document.createElement("div");
      msg.style.cssText = "text-align:center;padding:1.5rem;color:var(--text-muted);";
      msg.textContent = "Open a full share link (…/view.html?node=…#k=…) to watch a broadcast.";
  ...
  if (watcher) {
    ...
    // Content key FIRST: the #k= fragment from the share link (never sent to any server).
    // No #k= => we cannot decrypt, so there's nothing to watch.
    const fragKey = fragmentKeyFromHash();
    if (!fragKey) {
      console.warn("[crypto] no #k= key in the share link; cannot decrypt");
      const section = document.querySelector("#watch-view section");
      const msg = document.createElement("div");
      msg.style.cssText = "text-align:center;padding:1.5rem;color:var(--text-muted);";
      msg.textContent =
        "This stream is encrypted. Open the complete share link — it must include the #… key at the end — to watch.";
      section?.appendChild(msg);
      return;
    }

    // Resolve node id -> relay via DHT discovery + /api/edge. Poll until the broadcaster is
    // live (nothing published yet). There is no directory server — the DHT is the only surface.
    let route = await resolveNodeId(nodeId);
    if (!route) {
      const section = document.querySelector("#watch-view section");
      ...
      while (!route && !stopped) {
        ...
        route = await resolveNodeId(nodeId);
      }
      waitingEl.remove();
      if (stopped || !route) return;
    }
    console.log(`[watch-timing] resolved @ ${ms()} -> ${route.relayUrl}`);

    // Derive the content key. The broadcaster sets the stream salt at go-live, which can land
    // shortly after resolve — poll briefly until it appears, then derive CK.
    let salt = await getSalt(nodeId);
    let waited = 0;
    while ((!salt || !salt.stream) && waited < 30000) {
      await new Promise((r) => setTimeout(r, 1000));
      waited += 1000;
      salt = await getSalt(nodeId);
    }
    if (!salt || !salt.stream) {
      console.warn("[crypto] stream salt unavailable; cannot derive the media key");
      return;
    }
    armViewer();
    await deriveMediaKey({
      fragmentKeyB64: fragKey,
      globalSaltB64: salt.global,
      streamSaltB64: salt.stream,
      streamId: nodeId,
      epoch: salt.epoch,
    });
    console.log(`[crypto] media key derived (epoch ${salt.epoch})`);

    // Re-derive on rotation: a broadcaster "reset key" bumps the epoch and the operator's
    // global kill switch changes the global salt — either changes the signature. Poll and
    // re-derive so the stream keeps playing for viewers who can still fetch the new salt.
    let installedSig = `${salt.global}|${salt.stream}|${salt.epoch}`;
    const saltPoll = window.setInterval(async () => {
      const s = await getSalt(nodeId);
      if (!s || !s.stream) return;
      const sig = `${s.global}|${s.stream}|${s.epoch}`;
      if (sig !== installedSig) {
        installedSig = sig;
        console.log(`[crypto] salt changed (epoch ${s.epoch}); re-deriving media key`);
        await deriveMediaKey({
          fragmentKeyB64: fragKey,
          globalSaltB64: s.global,
          streamSaltB64: s.stream,
    ...
    window.addEventListener("beforeunload", () => clearInterval(saltPoll));

    // Relay-blind badge overlay (infrastructure property — no one in the path can decode).
    ...
    // Connect + subscribe under the DHT record's name (the bare node id). relay_url ends in
    // "/", so "?jwt=" appends cleanly; the jwt is the subscribe-scoped token from /api/edge.
    setActiveRelay(route.relayUrl);
    watcher.setAttribute("name", route.broadcast);
    watcher.setAttribute("url", route.jwt ? `${route.relayUrl}?jwt=${route.jwt}` : route.relayUrl);
    console.log(`[node-watch] subscribed name=${route.broadcast} url=${route.relayUrl} jwt=${route.jwt ? "yes" : "no"} @ ${ms()}`);
    void gotFirstFrame; // first-frame probe is informational on the node path
