...
// is the one thing this product exists not to do. Writing it out is also what keeps this page
// inside its own content-security policy, which permits no external script — and what keeps
// earthseed's runtime dependency count at zero, which is the claim INTEGRITY.md rests on.
//
...
// screen, not by capacity (see QR_MODULE_MIN_PX in compositor.js).
...
/**
 * A finished QR: a square grid where true means a dark module. Out-of-range coordinates read
 * light, so the quiet zone the caller must leave around it comes free.
 * @typedef {{size:number, version:number, get:(x:number,y:number)=>boolean}} QrMatrix
 */
 ...
 * @param {number} degree @returns {Uint8Array}
 */
 ...
 * @param {Uint8Array} data @param {Uint8Array} divisor @returns {Uint8Array}
 */
 ...
 * tabulated.
 * @param {number} ver @returns {number}
 */
 ...
 * @param {number} ver @param {EccLevel} ecl @returns {number}
 */
...
class Builder {
  ...
  /** @param {number} len @param {number[]} history */
  pushRun(len, history) {
  ...
  /** @param {boolean} runColor @param {number} runLen @param {number[]} history @returns {number} */
  endRun(runColor, runLen, history) {
    if (runColor) {
      this.pushRun(runLen, history);
      runLen = 0;
    }
 ...
 *
 * @param {string} text
 * @param {{ecl?:EccLevel, maxVersion?:number}} [opts]
 ...
 *
 * @param {Uint8Array} data @param {number} ver @param {EccLevel} ecl @returns {Uint8Array}
 */
