...
// second" one and the burn-in becomes the thing that is lying. So this does not trust the local
// clock at all; it derives UTC from our own edge and keeps it honest three ways.
//
...
//
// The offset is measured against performance.now(), never Date.now(), and the burned-in time is
// performance.now() + offset. Date.now() is whatever the operating system currently believes:
// an NTP daemon stepping the system clock mid-broadcast, or a user changing the timezone, would
// silently corrupt every frame after it while the offset still looked freshly measured.
// performance.now() is monotonic and immune to both. This is the fix that matters most, and it
// costs nothing.
//
// 3. SLEW, NEVER STEP
//
// Each resync produces a slightly different offset (measurement noise, real drift). Applying it
// directly would make the burned-in clock jump — and a clock that jumps BACKWARDS in something
// presented as a record of when things happened is worse than one that is quietly a few
// milliseconds off. So corrections are slewed in at a bounded rate rather than stepped. At 1%
// the displayed clock stays strictly increasing (the rate never drops below 0.99x, which no
...
// common-mode error cancels — see `uncertaintyMs` and the note in geo-stamp.js.

/**
 * What /api/whereami answers with: the caller's own coarse location, and the edge's clock.
 * @typedef {{lat:number|null, lon:number|null, city:string|null, region:string|null,
 *            country:string|null, colo:string|null, server_time_ms:number,
 *            source?:string, precision?:string}} WhereAmI
 */
...
async function sample() {
  ...
  // midpoint of the local monotonic timebase; the error is at most half the round trip, which is
  // why the caller keeps the sample with the smallest one.
  //
...
async function burst(n) {
  /** @type {{offsetMs:number, rttMs:number, data:WhereAmI}|null} */
  let best = null;
  for (let i = 0; i < n; i++) {
    try {
...
/** @returns {Promise<EdgeClock>} */
export async function createEdgeClock() {
  ...
  /** @type {number|null} */ let steppedAt = null;
  ...
  const resync = () => {
    void burst(BURST_RESYNC).then((s) => {
      ...
      latest = s.data;
      // Only accept an offset measured at least as cleanly as what we are already using; a
      // congested burst would otherwise drag a good anchor toward a worse one.
  ...
  return {
    // The offset is slewed on every call, so `at` and `now` share one continuously corrected
    // mapping. Stamping a past instant is therefore just arithmetic — no second measurement, and
    // no drift between "when the frame arrived" and "when we asked".
    //
    // `at` is the one that matters: it lets a caller stamp a moment that has already passed —
    // the instant a video frame was captured — rather than the moment it got around to asking.
    // requestVideoFrameCallback reports captureTime in exactly this timebase, which is why the
    // anchor is performance.now().
