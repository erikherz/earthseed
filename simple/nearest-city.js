// Turns a coarse Cloudflare geo (state/province name + lat/lon) into a friendly, deliberately
// APPROXIMATE location string like "~10 mi from NYC, NY".
//
// Approximate by design: the distance is measured to the LARGEST city of the region in question
// (NYC for New York — not the capital, Albany) and rounded to the nearest 5 miles, so an exact
// position is never revealed. Where the region cannot be placed it degrades to the state, then
// the country's largest city, then the bare country.
//
// WHERE THIS IS USED HERE, AND WHERE IT IS NOT
//
// It is UI copy, and only UI copy: it tells a broadcaster, before they switch the burn-in on,
// roughly what the network fallback would say about them. It never reaches the burned-in line
// itself — see geo-stamp.js, where the fallback deliberately prints raw four-decimal coordinates
// under a `~city` marker rather than a friendly name, because a name is easy to mistake for a
// fix and the whole design rule there is that the two sources must never render alike.
//
// Ported from wallflower/src/geo/nearest-city.ts, where it is imported by main.ts and never
// called — the viewer-location surface it was written for was removed before it shipped. Porting
// it unused would have carried dead code into a public repo, so it has a caller here or it would
// not be here at all.

/**
 * The reference city for a region.
 * @typedef {{name:string, label:string, lat:number, lon:number}} City
 *   name  — display name of the largest city
 *   label — compact region tag shown after it (state code / country code)
 */
...
const US_STATE_ABBR = Object.fromEntries(
  Object.entries(US_STATES).map(([name, c]) => [name, c.label])
);
const CA_PROVINCE_ABBR = Object.fromEntries(
...
/** @param {string|null} country @param {string|null} region @returns {City|null} */
function referenceCity(country, region) {
 ...
 *
 * @param {string|null} country @param {string|null} region
 * @param {string|number|null} latStr @param {string|number|null} lonStr
 * @returns {string}
 */
export function describeLocation(country, region, latStr, lonStr) {
  const lat = latStr != null ? parseFloat(String(latStr)) : NaN;
  const lon = lonStr != null ? parseFloat(String(lonStr)) : NaN;
