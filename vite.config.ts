...
export function moqWebTransportOnly(): Plugin {
...
export function mediaCryptoPatch(): Plugin {
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      // pkarr's DHT record encoder (dns-packet) does `require('buffer').Buffer`. Without
      // this alias the browser build stays green but Buffer is undefined at runtime, so
      // the DHT publish/resolve path dies. Alias to the npm `buffer` polyfill so watch-by-
      // pubkey works. (src/crypto/dht.ts also installs globalThis.Buffer as a belt-and-braces
      // shim for pkarr internals that read the global without importing it.)
      buffer: "buffer/",
    },
  },
  build: {
